...
public class Decorators {
	...
	private static final Consumer<DashboardPage> superuserComponentPresent = (dp) -> shouldDisplay(
	...
	private static final Consumer<DashboardPage> guestComponentNotPresent = (dp) -> shouldNotDisplay(
			dp.getGuestComponents());
	private static final Consumer<DashboardPage> superuserComponentNotPresent = (dp) -> shouldNotDisplay(
			dp.getSuperUserComponents());
	...
	public static final Consumer<DashboardPage> guestPage = guestSelection.andThen(guestComponentPresent)
			.andThen(adminComponentNotPresent).andThen(superuserComponentNotPresent);
	public static final Consumer<DashboardPage> suPage = superUserSelection.andThen(superuserComponentPresent)
			.andThen(guestComponentPresent).andThen(adminComponentNotPresent);
